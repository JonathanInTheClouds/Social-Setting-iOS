...
extension Network {
    ...
    func getCommentFeed(id: Int, page: Int) -> AnyPublisher<PostDetailResponseModel, Error> {
        var component = self.component
        component.path = "/api/subSetting/*/post/id/\(id)/comment"
        component.queryItems = [
            URLQueryItem(name: "page", value: "\(page)"),
            URLQueryItem(name: "amount", value: "10"),
            URLQueryItem(name: "profileInfo", value: "\(true)")
        ...
        return URLSession.shared.dataTaskPublisher(for: request)
            ...
            .decode(type: PostDetailResponseModel.self, decoder: JSONDecoder.decoder)
            .receive(on: DispatchQueue.main)
            .eraseToAnyPublisher()
    }
}
